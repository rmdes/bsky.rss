...
test("runBenchmark runs N synthetic bots against the local mock server and reports plausible RSS samples", async () => {
  ...
  assert.ok(report.steadyStateRssBytes > 0, "steady-state RSS must be a real, positive measurement");
  assert.ok(report.peakRssBytes >= report.steadyStateRssBytes, "peak must be at least the steady-state value");

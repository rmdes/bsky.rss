...
test('importOneBot writes bot.json and config.json, stripping the dead publishDate field', t => {
  ...
  const botJson = JSON.parse(
    readFileSync(join(configRoot, 'bots', 'actudroit-fr', 'bot.json'), 'utf-8'),
  );
  ...
  assert.equal(
    'publishDate' in configJson,
    false,
    'publishDate must be stripped, not carried over',
  );
});
  ...
  writeLegacyBot(
    ...
    {string: '$title'},
    {
      ...
      persistJson: {
        ...
        handle: 'actudroit.skyfleet.blue',
      },
    },
  );
...
test('importLegacyFleet skips a directory with no docker-compose.yml (not an independent bot)', t => {
  ...
  writeFileSync(
    join(sourceRoot, 'shared-trump-data', 'config.json'),
...
test("importLegacyFleet's --only filter merges into an existing secrets file instead of overwriting it", t => {
  ...
  assert.equal(
    secrets['bot-b'],
    'bot-b-password',
    "an --only run must not drop other bots' existing secrets",
  );
});

...
test("records each operational outcome in independent counters", () => {
  const operations = new BotOperations("bot-a", clock("2026-08-03T10:00:00.000Z", "2026-08-03T10:01:00.000Z"));
...
test("reports feed failure transitions and recovers after consecutive failures", () => {
  ...
  assert.deepEqual(operations.recordFeedFailure("timeout"), { becameFailing: true, consecutiveFailures: 1 });
  assert.deepEqual(operations.recordFeedFailure("dns"), { becameFailing: false, consecutiveFailures: 2 });
  assert.deepEqual(operations.recordFeedFailure("parse"), { becameFailing: false, consecutiveFailures: 3 });
...
test("classifies expected feed failure shapes without retaining error details", () => {
  ...
  const operations = new BotOperations("bot-a", clock("2026-08-03T10:00:00.000Z"));
  operations.recordFeedFailure(classifyFeedFailure({ status: 500, url: "https://secret.example/feed.xml" }));
  const snapshot = operations.snapshot();

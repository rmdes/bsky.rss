...
function setupFleet(): { configRoot: string; secretsFilePath: string; dataRoot: string; cleanup: () => void } {
  ...
  return { configRoot, secretsFilePath, dataRoot, cleanup: () => rmSync(root, { recursive: true, force: true }) };
}
...
test("loadFleet reports one bad bot directory as an error without dropping the other bots", (t) => {
  ...
  writeFileSync(join(configRoot, "bots", "bot-bad", "config.json"), JSON.stringify({ string: "$title" }));

...
test('waits at least staggerSeconds between activations', async () => {
  ...
  assert.ok(
    elapsedMs >= staggerSeconds * 1000,
    `expected at least ${staggerSeconds * 1000}ms, got ${elapsedMs}ms`,
  );
});
...
test('does not wait after the last bot', async () => {
  ...
  assert.ok(
    elapsedMs < staggerSeconds * 1000,
    'a single bot must not incur a trailing stagger wait',
  );
});
...
test('abortActivation stops the loop before activating any not-yet-activated bot', async () => {
  ...
  assert.ok(
    coordinator.activeWorkers().length < 3,
    'abort must prevent at least one remaining bot from activating',
  );
});
...
test('abortActivation interrupts an in-progress stagger wait immediately rather than after the full delay', async () => {
  ...
  assert.ok(
    elapsed < 1000,
    `abort should interrupt the stagger wait immediately, took ${elapsed}ms`,
  );
});
...
test('activation summaries are privacy-safe while debug retains failure detail and stored status is unchanged', async () => {
  ...
  assert.ok(
    records.some(
      record =>
        record.level === 'debug' &&
        record.botId === 'failed-bot' &&
        record.message.includes('private raw login failure'),
    ),
  );

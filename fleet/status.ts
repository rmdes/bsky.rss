...
export function formatFleetStatus(
  ...
  if (snapshot.bots.feedsOk > 0) {
    botFacts.push(`${formatInteger(snapshot.bots.feedsOk)} feed${plural(snapshot.bots.feedsOk)} ok`);
  }
  if (snapshot.bots.feedsFailing > 0) {
    botFacts.push(`${formatInteger(snapshot.bots.feedsFailing)} feed${plural(snapshot.bots.feedsFailing)} failing`);
  }
  if (snapshot.bots.feedsStarting > 0) {
    botFacts.push(`${formatInteger(snapshot.bots.feedsStarting)} feed${plural(snapshot.bots.feedsStarting)} starting`);
  }
...
function formatBotStatus(bot: FleetBotStatus): string {
  const counters = counterNames.map(
    (name) => `${name}=${formatInteger(bot.counters[name])}`
  );
  return [
    `Bot ${bot.botId}`,
...
function isFeedFailureCategory(value: unknown): value is FeedFailureCategory | null {
  return value === null || value === "timeout" || value === "dns" || value === "tls" ||
    value === "connection" || value === "parse" || value === "other" ||
    (typeof value === "string" && /^http-\d+$/.test(value));
}

function hasNumericProperties(value: unknown, names: readonly string[]): value is Record<string, number> {
...
function isFleetBotStatus(value: unknown): value is FleetBotStatus {
  return isRecord(value) && typeof value.botId === "string" &&
    isActivationState(value.activationState) && isFeedState(value.feedState) &&
    isNullableTimestamp(value.lastFeedSuccessAt) && isNullableTimestamp(value.lastFeedFailureAt) &&
    ...
    isFleetLogLevel(value.effectiveLogLevel) && isNullableTimestamp(value.logOverrideExpiresAt);
}

function isFleetStatusSnapshot(value: unknown): value is FleetStatusSnapshot {
  if (!isRecord(value) || value.schemaVersion !== 1 || !isFleetPhase(value.phase) ||
    !isTimestamp(value.startedAt) || !isTimestamp(value.heartbeatAt) ||
    !hasNumericProperties(value.bots, [
      "configured", "active", "activationFailed", "configInvalid",
      "feedsStarting", "feedsOk", "feedsFailing",

...
test("isEligibleNow is true before any post has been recorded", () => {
  const s = new Scheduler({ minSpacing: 1, maxSpacing: 60, spacingWindow: 600, adaptiveSpacing: true });
  assert.equal(s.isEligibleNow(1), true);
});

test("minSpacing blocks posting immediately after a post", () => {
  const s = new Scheduler({ minSpacing: 60, maxSpacing: 60, spacingWindow: 600, adaptiveSpacing: false });
  s.recordPost();
...
test("computeDelaySeconds is bypassed entirely when adaptiveSpacing is false", () => {
  const s = new Scheduler({ minSpacing: 5, maxSpacing: 60, spacingWindow: 600, adaptiveSpacing: false });
  s.recordPost();
...
test("rate-limit deadline overrides spacing once set", () => {
  const s = new Scheduler({ minSpacing: 1, maxSpacing: 60, spacingWindow: 600, adaptiveSpacing: false });

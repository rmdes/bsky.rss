...
export function isStillFresh(itemDate: string, maxItemAgeMinutes: number, now: Date = new Date()): boolean {
...
export function selectEligibleItems(
  ...
  const newestFirst = [...fresh].sort((a, b) => new Date(b.itemDate).getTime() - new Date(a.itemDate).getTime());
  const kept = newestFirst.slice(0, config.maxCatchupItems);
  const overflowed = newestFirst.slice(config.maxCatchupItems);

  const toPublish = [...kept].sort((a, b) => new Date(a.itemDate).getTime() - new Date(b.itemDate).getTime());

...
export function parseComposeEnv(composeYaml: string): ParsedComposeEnv {
  ...
  if (!identifier || !appPassword || !instanceUrl || !feedUrl) {
    throw new Error("docker-compose.yml is missing one of IDENTIFIER/APP_PASSWORD/INSTANCE_URL/FETCH_URL");
  }

  const fetchIntervalMinutes = env.FETCH_INTERVAL ? Number(env.FETCH_INTERVAL) : DEFAULT_FETCH_INTERVAL_MINUTES;

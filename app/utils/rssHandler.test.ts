...
describe('rssHandler', () => {
  ...
  describe('HTML entity decoding', () => {
    ...
    it('should handle mixed text and entities', () => {
      ...
      const expected = 'Tom & Jerry: A Classic'; //   is non-breaking space
    ...
    it('should handle numeric character references', () => {
      ...
      const input = '&#8220;Hello&#8221;'; // Smart quotes
      const result = decode(input);

      assert(result.includes('“')); // Left double quote
      assert(result.includes('”')); // Right double quote
    });
  });

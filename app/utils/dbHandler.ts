...
async function readLast() {
  if (!fs.existsSync(import.meta.dirname + '/../../data/last.txt')) {
    ...
    const data = fs.readFileSync(import.meta.dirname + '/../../data/last.txt', 'utf8');
    return data;
  }
}

async function writeDate(date: Date) {
  fs.writeFileSync(import.meta.dirname + '/../../data/last.txt', date.toISOString(), 'utf8');
  return date;
}

async function readPersistData() {
  if (!fs.existsSync(import.meta.dirname + '/../../data/persist.json')) {
    fs.writeFileSync(import.meta.dirname + '/../../data/persist.json', JSON.stringify({}), 'utf8');
    return {};
  } else {
    const data = fs.readFileSync(import.meta.dirname + '/../../data/persist.json', 'utf8');
...
async function writePersistDate(persistData: object) {
  fs.writeFileSync(
    import.meta.dirname + '/../../data/persist.json',
...
async function initConfig() {
  try {
    const data = fs.readFileSync(import.meta.dirname + '/../../data/config.json', 'utf8');
...
async function valueExists(value: string) {
  if (!fs.existsSync(import.meta.dirname + '/../../data/db.txt')) {
    fs.writeFileSync(import.meta.dirname + '/../../data/db.txt', '', 'utf8');
    return false;
  } else {
    const fileContent = fs.readFileSync(import.meta.dirname + '/../../data/db.txt', 'utf8');
...
async function writeValue(value: string) {
  ...
  fs.appendFileSync(
    import.meta.dirname + '/../../data/db.txt',
...
async function cleanupOldValues() {
  ...
  const currentDate = new Date();
  const oldFileContent = fs.readFileSync(import.meta.dirname + '/../../data/db.txt', 'utf8');
  let newFileContent = '';
  ...
  fs.writeFileSync(import.meta.dirname + '/../../data/db.txt', newFileContent, 'utf8');
  return true;
}

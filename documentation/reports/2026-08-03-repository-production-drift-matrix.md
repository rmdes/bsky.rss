...
| Capability | Production (`/home/skyfleet-next`) | `bsky.rss` | Fleet template | Required action |
|---|---|---|---|---|
...
| Configuration | **Observed:** 59 enabled bot configs; fleet settings match both repositories' examples. Production also declares an extra-CA environment key. **Unverified:** current dry-run/publishing mode, because environment values were deliberately not extracted. | Owns config loader and examples; no schema-backed validator yet. Runtime fleet example includes extra-CA support. | Example fleet values match production/runtime; no validator and no extra-CA operator contract. | Preserve existing shapes while adding schema version/validation. Model extra CA as a documented optional operator overlay, not a hard-coded production identity. |
...
| Queue behavior | **Observed:** production configuration sets a per-bot queue ceiling of 500, a 60-second drain, 5-item/120-minute freshness, and 60–300-second adaptive spacing for 57 bots. A follow-up rolling-log aggregation found 187 feed retrieval errors across 3 anonymous bots: 185 HTTP 500 responses from one bot/feed with zero queued/posted events, while the other two continued (18/16 and 1/1 queued/posted). Separately, 196 caught Open Graph failures spanned 22 anonymous bots; item-handler errors were zero. | Implements persistent statuses, dedupe, freshness, capacity, adaptive spacing, rate limits, and conservative uncertain-result skip. Source distinguishes feed retrieval from caught Open Graph fallback. | Only supplies matching example settings; no queue diagnostics or validation. | Preserve authoritative runtime behavior during health/schema work; add aggregate status without identifiers/content, distinguish feed retrieval alerts from caught Open Graph fallback, and test exact edge cases. |
...
| Shutdown | **Observed:** static contract has 45-second Compose grace and direct Node; source has 30-second overall bounded shutdown and lock release. **Unverified:** live production shutdown behavior. | Fleet handlers exist; solo parity and lifecycle tests are pending. | Declares direct Node and 45-second grace. | Test signal handling, queue persistence, SQLite close, and lock release before claiming production shutdown verification. |
...
10. **Make production adoption a separate, explicitly authorized operation.** The implementation plans may create and validate artifacts, but must not imply deployment to the production host, publishing changes, or container lifecycle actions are part of automated acceptance.

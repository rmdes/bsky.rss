...
- Summary logs never contain URLs, titles, post content, or raw errors. Debug may contain operational URLs/titles/content, but code must never pass credentials, sessions, tokens, app passwords, or complete config objects to the logger.
...
- `formatDebugError` emits only an error name, message, and stack; it does not serialize arbitrary properties such as `config`, `headers`, `session`, or `password`.
...
`formatDebugError` may read `name`, `message`, and `stack` from an `Error`. For a non-`Error`, return its primitive string form only; never `JSON.stringify` arbitrary objects.
...
- debug contains the thrown error stack for only that bot.

Change `BskyClient` to require a `FleetLogger`. Test that login and dry-run/current post messages follow the levels without including account handles at summary. Keep `classifyPostError`, record creation, and return values unchanged; `BotWorker`, not `BskyClient`, owns outcome counters.

Add `logger: FleetLogger` to `AuthCoordinatorOptions`. Assert successful/failed activation uses summary, raw activation details use debug, and failure storage still retains the existing string for internal status counts.
...
Do not move branches or change status mutations. Add one counter call immediately beside each existing queue/post decision. Replace direct runtime `console.log` calls in the named files with the shared logger. Do not change standalone diagnostic CLI output beyond supplying a debug logger where constructors now require one.
...
Tests cover missing, malformed, wrong `schemaVersion`, current, stale, and stopping files. A heartbeat older than 150 seconds is stale. Zero denominators print `n/a`, never `NaN` or `Infinity`. The default format contains no bot rows. `--bots` rows contain only the approved fields. Posts use `successful / (successful + uncertain) terminal outcomes`; deferred and exception counts are separate because those rows remain queued.
...
Test duration suffixes `s`, `m`, and `h`; reject zero, negative, fractional, missing-unit, overflow, and unknown-unit values. Override documents are all-or-nothing: one unknown bot, invalid level, invalid timestamp, or non-object entry rejects the entire replacement.
...
Allow all three valid levels so an individual bot can also be lowered below a more verbose global setting. `set` and `clear` load bot IDs from the current `status.json`; unknown IDs make no file change. `list` filters expired entries and prints bot ID, level, expiry, and remaining duration. All mutations prune expired entries and call `writeOverrides`, which delegates to `writePrivateJsonAtomic`.

Test set/list/clear, unknown bot, missing/malformed status, invalid level/duration, exact expiry, mode `0600`, and no file mutation on any rejected command.
...
- log delta summary every 5 minutes and replace the previous counter baseline;
...
- catch snapshot/override observer errors, log one safe warning plus debug detail, and never throw into bot execution.

Fake-timer tests prove the immediate startup snapshot exists before activation finishes, 59-bot starting counts are visible, each cadence fires only its own action, and stopping is written before the fake worker shutdown resolves. No real signals, sleeps, authentication, feeds, or posts.
...
8. On config errors, summary logs only `[bot-id] Config invalid`; debug logs the stored detail. Never log the loaded `BotSpec` or secrets object.
9. At global debug startup, emit one privacy warning.
10. Preserve existing PID-lock acquisition/release, no-active-bots exit, signal ordering, bounded shutdown, and dry-run behavior.
...
Expected: the first search finds no serialized private fields (type/import names in tests must be inspected, not blindly accepted); the second finds no server, metrics service, or persistence addition; only `bsky.rss` implementation/docs paths are changed.
...
- full verbose/debug remains available;
- one bot can change level without restarting or affecting the other bots;
- set/detect/expire/list/clear and malformed-file retention work;
- snapshot/override atomic replacement and `0600` are tested;
- starting/current/stale/stopping status cases are explicit;
